...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift (del_x, del_y, e1) => case eval_prog(e1, env) of
					NoPoints => NoPoints
				      | Point(x, y) => Point(x + del_x, y + del_y)
				      | Line(m, b) => Line(m, b + del_y - del_x * m)
				      | VerticalLine(x) => VerticalLine(x + del_x)
				      | LineSegment(x1, y1, x2, y2) => LineSegment(x1 + del_x, y1 + del_y,
										   x2 + del_x, y2 + del_y)
				      | _ => raise Impossible("should not be here")
...
fun preprocess_prog e = 
	...
	LineSegment (x1, y1, x2, y2) => (if (real_close_point (x1, y1) (x2, y2)) 
					 ...
					 else if not(real_close(x1, x2)) andalso x1 > x2
					 then LineSegment(x2, y2, x1, y1)
					 else e)
      | Shift (a, b, x) => Shift(a, b, preprocess_prog x)
